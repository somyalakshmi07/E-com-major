...
const sessionConfig = {
  secret: process.env.SESSION_SECRET || 'BafmaYs4f6q6aLHkjPmPckBIKS3g6+VZR7zHOOIh3A8=',
  resave: false,
...
// Products database (temporary until MongoDB integration)
const products = [
...
app.use((req, res) => {
  res.status(404).render('errors/404', { 
    ...
    pageTitle: 'Page Not Found - Little Treasures',
    cartCount: req.session.cart ? req.session.cart.reduce((sum, item) => sum + (item.quantity || 0), 0) : 0,
...
app.use((err, req, res, next) => {
  ...
  res.status(err.status || 500).render('errors/500', { 
    title: 'Server Error',
    pageTitle: 'Server Error - Little Treasures',
    error: process.env.NODE_ENV === 'development' ? err : null,
    cartCount: req.session.cart ? req.session.cart.reduce((sum, item) => sum + (item.quantity || 0), 0) : 0,
    wishlistCount: req.session.wishlist ? req.session.wishlist.length : 0
  });
